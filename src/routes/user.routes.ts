...
userRouter.post('/create', (request: Request, response: Response) => {
  const { userName, userEmail, userCPF, userPhone, userPassword } =
    request.body;
  ...
  if (!cpfRegex.test(userCPF)) {
    return response
      .status(400)
      .json({ message: 'CPF inválido. Formato esperado: XXX.XXX.XXX-XX' });
  }
  ...
  const usuario = createService.createUser({
    userName,
    userEmail,
    userCPF,
    userPhone,
...
userRouter.put('/update/:id', (request: Request, response: Response) => {
  ...
  const updatedUser = updateService.update(
    request.params.id,
    userName,
    userEmail,
    userCPF,
